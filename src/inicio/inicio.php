...
<main>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center;min-height:calc(100vh - 200px);padding-top:20px">
    <div>
      <div style="display:inline-flex;align-items:center;gap:7px;background:rgba(201,146,42,.08);border:1px solid rgba(201,146,42,.18);border-radius:999px;padding:5px 14px;font-family:var(--ff-m);font-size:1rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--gold);margin-bottom:28px">
      ...
      <h1 style="font-family:var(--ff-d);font-size:clamp(3.8rem,8.5vw,7rem);font-weight:900;line-height:.9;letter-spacing:-.04em;color:#000000;margin-bottom:24px">
        Sur<br>Arte<br><em style="font-style:italic;color:var(--clay)">Andino</em>
      </h1>
      <p style="font-size:clamp(1.2rem,1.9vw,1.55rem);font-weight:550;color:#000000;max-width:440px;line-height:1.82;margin-bottom:36px">
    ...
    <div style="font-weight:550;display:flex;flex-direction:column;gap:14px">
  <?php foreach ([
    ...
    <?php if ($ic === '🎨'): ?>
      <div>
        <h1 style="font-family:var(--ff-d);font-size:clamp(1.7rem,5.4vw,3.8rem);font-weight:900;line-height:.9;letter-spacing:-.04em;color:#000000;margin-bottom:24px">Información:
      </h1></div>
    <?php endif; ?>
    <div style="background:#fff;border:1px solid var(--cream-dk);border-radius:14px;padding:20px 24px;display:flex;align-items:center;gap:18px;transition:border-color .25s,box-shadow .25s,transform .25s" class="stat-card">
      ...
      <div>
        ...
        <div style="font-family:var(--ff-m);font-size:.92rem;letter-spacing:.12em;text-transform:uppercase;color:#111111;margin-top:3px"><?= $lbl ?></div>
      </div>
    </div>
  <?php endforeach; ?>
</div>
